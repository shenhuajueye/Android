...
public class MyContentAdapter extends  RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public MyContentAdapter(Context context,List<Post> data){
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.mycontent_item,viewGroup,false);
        View footView = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.foot_item,viewGroup,false);
        if(viewType == F_TYPE){
            return new MyContentAdapter.RecyclerViewHolder(footView,F_TYPE);
        }else {
            return new MyContentAdapter.RecyclerViewHolder(view,N_TYPE);
        }
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(isFootView&&getItemViewType(position)==F_TYPE){
            //底部加载提示
            final MyContentAdapter.RecyclerViewHolder recyclerViewHolder = (MyContentAdapter.RecyclerViewHolder) holder;
    ...
    private class RecyclerViewHolder extends RecyclerView.ViewHolder {
        public TextView username,nickname,content,time; //community_item的TextView
        public TextView loading;
        public RecyclerViewHolder(View itemView, int view_type) {
            super(itemView);
            if(view_type==N_TYPE){
                username =itemView.findViewById(R.id.myContentUsername);
                nickname = itemView.findViewById(R.id.myContentNickname);
                content = itemView.findViewById(R.id.myContent);
                time = itemView.findViewById(R.id.myContentTime);
